...
if (empty($action)) {
	...
	<div class='broucherrequest-top'></div>
	...
	if($name != "" && $email != "")
	{
		...
		$message = "<table cellspacing= 15 >";
			...
			$message .= "<tr><td>Requested Programme</td><td>".$programme-requested."</td></tr>";
		$message .= "</table>";
		...
		if($db->insert($tblname,$record) > 0) 
		{
			...
			{	
				$send = $mail->Send_Email($email,$email,$to[$i]['email'],$to[$i]['email'],"broucherrequest us request",$message,MAILSERVER);
			}
