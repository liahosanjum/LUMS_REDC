 ...
 */
 ...
 class Frontpaginate{
      ...
      */
     function Frontpaginate($limit="", $num="", $url="", $maxPages=""){
	...
	function calculate_numPages(){
     ...
     function getPreviousPage(){
         $qrstr=$this->start-$this->limit;
		 if($qrstr==1)
		 {
		 	$qrstr=0;
		 }
		 if ($this->currentPage > 1){
             return "<a class='' href='".$this->url.$this->urlAppendString.$this->extraQuerysting."sc=".$this->sortcolumn."&sd=".$this->sortdirection."&start=".
             ($qrstr).
             "' title='Previous page'><img src='images/left_arow.gif' style='border:0px;' /></a>";
         }
     }
     ...
     function getNextPage(){
		 ...
		 if ($this->currentPage < $this->numPages){
             return "<a  class='' href='".$this->url.$this->urlAppendString.$this->extraQuerysting."sc=".$this->sortcolumn."&sd=".$this->sortdirection."&start=".
             ($starts+$this->limit).
             "' title='Next page'><img src='images/right_arow.gif' style='border:0px;' /></a>";
         }
     }
     ...
     function getFirstPage(){
         if ($this->currentPage > 1){//If not on first page already
             return "<a class='' href='".$this->url.$this->urlAppendString.$this->extraQuerysting."sc=".$this->sortcolumn."&sd=".$this->sortdirection."&start=0' 
             title='First page'><img src='images/move_first.gif' style='border:0px;' /></a>";
         }
     }
     ...
     function getLastPage(){
         ...
         if ($this->currentPage < $this->numPages){//If not on last page already
             ...
             $start_val = (($this->num - fmod($this->num, $this->limit)) );
             
             if ($start_val >=$this->num){
             	$start_val = $start_val - $this->limit;
             }
            
             return "<a class='' href='".$this->url.$this->urlAppendString.$this->extraQuerysting."sc=".$this->sortcolumn."&sd=".$this->sortdirection."&start=".$start_val
             ."' title='Last page'><img src='images/move_last.gif' style='border:0px;' /></a>";
         }
     }     
     ...
     function getPaginationLinks(){
         ...
         for ($i=$pageStart;$i<=$pageEnd;$i++){
             //echo($i."=".$this->currentPage."<br>");
			 if ($i == $this->currentPage){
                 $ar[$i] = "<span class='pagination_for_class' href='#'>" .$i."</span>";
             } else {
                 if($i==1)
				 {
					 //echo("hello");
					 $ar[$i] = "<a class='' href='".$this->url.$this->urlAppendString.
					 $this->extraQuerysting."sc=".$this->sortcolumn."&sd=".$this->sortdirection."&start=".(((($i-1)*$this->limit)))."' 
      ...
      * @return Array (html strings)      
      */
     function getPortfolioPaginationLinks(){
         //Don't get carried away with paginating large sets of data.
         //This sets reasonable limits
         ...
         for ($i=$pageStart;$i<=$pageEnd;$i++){
             //echo($i."=".$this->currentPage."<br>");
			 if ($i == $this->currentPage){
                 $ar[$i] = "<span class=''> [".$i."] </span>";
             } else {
					$indx = strpos($this->url, 'cid') + 4;
//					echo substr($this->url, $indx, 5) ;
//					$length =  strpos($this->url, '&amp;', strpos($this->url, 'cid')) - $indx;
 					 $cid = substr($this->url, $indx) ;

                 if($i==1)
				 {
					$startindx = trim(((($i-1)*$this->limit)));
					 $ar[$i] = "<a class='' href=javascript:loadPortfolioTab('$cid','$startindx') title='Go to page ". $i ."'>".$i."</a>";
				 }else{
						$startindx = trim(((($i-1)*$this->limit)));
					 $ar[$i] = "<a class='' href=javascript:loadPortfolioTab('$cid','$startindx') title='Go to page ". $i ."'>".$i."</a>";
				 }	 
     ...
     function displayTable(){
         ...
         reset($arLinks);
         if (isset($first) && $first != ""){
            $output .= "<li class=\"pad\">".$first."</li>";
         }
         if (isset($previous) && $previous != ""){
             $output .= "<li class=\"pad\">".$previous."</li>";
         }
         ...
         while(list($i, $v) = each($arLinks)){
			 ...
			 if($i == count($arLinks) )
			 {
				$output .= "<li class=\"last\">".$arLinks[$i]."</li>";	 
			 }
			 else
			 {
				 $output .= "<li>".$arLinks[$i]."</li>";
     ...
     function displayPortfolioTable(){
         ...
         $previous = $this->getPreviousPage();
         $next = $this->getNextPage();*/
         $arLinks = $this->getPortfolioPaginationLinks();
