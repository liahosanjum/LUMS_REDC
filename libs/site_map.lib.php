...
class Sitemap extends db{
   ...
   function getSectionName($section_id)
		 ...
		 function getsectionimage()
	{
		...
		$query = "select sec_image from redc_page_section where sectionname= 'Site Map'";
    ...
    function displayGird($data = array()) {
		...
		$this->tpl->assign('pagedata', $this->getpagedata('Site Map'));
		$this->tpl->assign('simage', $this->getsectionimage());
