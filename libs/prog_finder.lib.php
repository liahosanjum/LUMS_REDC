...
//		$where .= " and (pro.enddate > '".date("Y-m-d")."' or pro.status='tba') and cat.oepcatid = pro.oepcatid AND isactive = 'Yes'";
		...
		if($formvars['month'] != "")
		{
			$_query	= "(select pro.*, cat.name as category_name from redc_oep_programmes as pro , redc_oep_programmes_category as cat  ".$where."  and (pro.enddate > '".date("Y-m-d")."' and pro.status='a') and cat.oepcatid = pro.oepcatid AND isactive = 'Yes' and pro.startdate is not null order by pro.startdate  asc )";
		}
		else
		{
			$_query	= "(select pro.*, cat.name as category_name from redc_oep_programmes as pro , redc_oep_programmes_category as cat  ".$where."  and (pro.enddate > '".date("Y-m-d")."' and pro.status='a') and cat.oepcatid = pro.oepcatid AND isactive = 'Yes' and pro.startdate is not null  )
union
(select pro.*, cat.name as category_name from redc_oep_programmes as pro , redc_oep_programmes_category as cat  ".$where."  and pro.status='tba' and cat.oepcatid = pro.oepcatid AND isactive = 'Yes')
		order by status, startdate  asc
";
		}
...
//$paging->num=$this->numrows("select pro.*, cat.name as category_name from " .$this->tablename . " as pro , ".$this->table_category. " as cat " .$where." AND iscompleted = 'N' AND isactive = 'Yes'");
    ...
    function displayForm($formvars = array()) {
        ...
        $this->tpl->display('prog_finder.tpl');
    }
    /**
