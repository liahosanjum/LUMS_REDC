...
class Apply extends db{
   ...
   function isvalidProgramme($pid)
   {
	...
	/*******************************************************************************************************/
			...
			if(strlen(trim($formvars['objectives'])) > 500) {
				$this->error["objectives"] = 'Please provide objectives with max of 500 characters';
				//return false; 
			}
		}
	...
	*/
	function sendEmail($formvars , $flag , $mailserver)
	{
		...
		else if($flag == 2)
		{
			...
			// fetch email content
			//echo "send mail to user on succesfful registration".
			...
			$fetch['fromname'].$fetch['fromemail'].$this->useremail.$this->useremail.$fetch['subject'].$email_body.$mailserver;
			//$send = $mail->Send_Email($fetch['fromname'],$fetch['fromemail'],$this->useremail,$this->useremail,$fetch['subject'],$email_body,$mailserver);
			

			 
				//$send = $mail->Send_Email($fetch['fromname'],$fetch['fromemail'],$this->useremail,$fetch['subject'],$email_body,$mailserver);
				
					$to        = $this->useremail;
					$subject   = $fetch['subject'];
					$body      = $email_body;
					$FromEmail = $fetch['fromemail'];
					//$FromEmail = "sa.aspire@gmail.com";
