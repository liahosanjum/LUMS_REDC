...
class Map extends db{
   ...
   function getSectionName($section_id)
	...
	function getsectionimage($sectionId)
	{
		...
		$query = "select sec_image from redc_page_section where psid=".$this->mySQLSafe($sectionId);
    ...
    function displayForm($formvars = array()) {
		...
		$this->tpl->display('map_direction.tpl');
    }
    /**
    ...
    function displayGird($data = array()) {
		...
		$this->tpl->assign('section_data', $this->getSectionName($_REQUEST['section_id']));
		$this->tpl->assign('pagedata', $this->getpagedata('Map Direction'));
