...
/***********************************************************************************************/
 	...
 	function sendForgotPassMail($uemail , $mailserver)
	{
		...
		$res = $this->select("select username , password from ".$this->usertblname." where username = '".$uemail."'");
		...
		//echo ($email_body);
		$email_body = str_replace('$$UserName$$', $res[0]["username"], $email_body);
		$email_body = str_replace('$$Password$$', $res[0]["password"], $email_body);
		$send = $mail->Send_Email($fetch['fromname'],$fetch['fromemail'],$res[0]['username'],$res[0]['username'],$fetch['subject'],$email_body,$mailserver);
	}
	...
	{
		$query = "select uid from ".$this->usertblname." where username = '".$username."' and password = '".$password."'";
	...
	*/
	function sendEmail($formvars , $flag , $mailserver)
	{
		...
		if($flag == 1)
		{
			...
			$email_body = str_replace('__USERNAME__', $formvars["username"], $email_body);
			
			$email_body = str_replace('__PASSWORD__', $formvars["password"], $email_body);
			
			$send = $mail->Send_Email($fetch['fromname'],$fetch['fromemail'],$formvars['username'],$formvars['username'],$fetch['subject'],$email_body,$mailserver);
...
//		$qryattendprog = "select p.name , a.oepaid from ".$this->tablename." as a , ".$this->table_name." as p where a.uid = $uid and a.oepid = p.oepid and a.oepaid != $aid and p.iscompleted = 'Y'";
     ...
     * @param array $formvars the form variables
     */
    function addEntry($formvars , $fk_uid) 
	{
		$fk_user_id = $fk_uid;
