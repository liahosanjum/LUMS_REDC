...
//	if (typeof programmeName == 'undefined') 
	{
		var list = document.getElementById('apply-oepprogrammes');
		
		if(list != null && list.type != 'hidden')
		{
			if(list.options[list.selectedIndex].text != "--select--")
			{
				programmeName = list.options[list.selectedIndex].text;
			}
		}
	}	
}
...
else if(uid > 0)
{
	if (typeof programmeName != 'undefined') 
	{
		page = "applyonline.php?pname= "+ programmeName;	
	}
	else
	{
...
if(oepid != "")
{
	//alert("alreadyApplied : "+alreadyApplied+" <---> "+"oepid : "+oepid)
	if(alreadyApplied == oepid)
	{
		page = "ifalreadyapp.php";
		h = 150;	
	}
}

var str = "<div style='width:100%; height:100%; margin-top: 134px; margin-left:50%; vertical-align:text-top; ' align='center'><img src='images/big.gif' border='0' /> Loading...</div>";
			...
			if (apply.validate()) {
				...
				$('#apply-container .apply-content').animate({
					...
					$('#apply-container .apply-loading').fadeIn(200, function () {*/
						...
						$.ajax({
							url: 'pages/applyonline.php',
							data: $('#apply-container form').serialize() + '&action=check&divnum='+getDivId()+"&pname=" + programmeName,
							type: 'post',
							cache: false,
							dataType: 'html',
							complete: function (xhr) {
								...
								$('#apply-container .apply-loading').fadeOut(200, function () {
									...
									if(msg == 1){
										...
										{											
											...
											window.location.href = callbackurl+'&cmd=1' + "&pname=" + programmeName;
											//toggleFunc($('#apply-container #apply-divname').val());
											//toggleFunc(1);
											counter++;
										}
...
*/						showLoginDiv();	
		...
		$('#apply-container .apply-password-link').click(function (e) {
			e.preventDefault();
/*				$('#apply-container .apply-message').fadeOut(function () {
					$('#apply-container .apply-message').removeClass('apply-error').empty();
				});
...
//									alert(res);
		...
		$('#apply-container .apply-password').click(function (e) {
			e.preventDefault();
			// validate form
			//$('#apply-container .apply-message').html('');
			if (apply.validatePassword()) {
				/*$('#apply-container .apply-message').fadeOut(function () {
					$('#apply-container .apply-message').removeClass('apply-error').empty();
				});
				$('#apply-container .apply-title').html('Loading...');
				...
				/*$('#apply-container .apply-content').animate({
					...
					//alert($('#apply-container form').serialize());
						...
						$.ajax({
							url: 'pages/applyonline.php',
							data: $('#apply-container form').serialize() + '&action=changepassword',
							type: 'post',
							cache: false,
							dataType: 'html',
							complete: function (abc) {
								...
								$('#apply-container .apply-loading').fadeOut(200, function () {
									...
									if(res == 1){										
										...
										if(counter == 0){
											showPasswordDiv();
											$('#apply-container .apply-message').show();
											$('#apply-container .apply-message').html($('<div class="apply-error">').append("Your password has been changed")).fadeIn(200);
...
//											toggleFunc(1);
									...
									{
//										$('#apply-container .apply-title').html("Change Password");
										showPasswordDiv();
										//$('#apply-container .apply-loading').hide();
										//$('#apply-container .apply-message').html("Invalid Email / Password.").fadeIn(200);	
										$('#apply-container .apply-message').show();
		...
		$('#apply-container .apply-forgotpass').click(function (e) {
			e.preventDefault();
			// validate form
			//$('#apply-container .apply-message').html('');
			if (apply.validateForgotPass()) {
				/*$('#apply-container .apply-message').fadeOut(function () {
					$('#apply-container .apply-message').removeClass('apply-error').empty();
				});
				$('#apply-container .apply-title').html('Loading...');
				...
				/*$('#apply-container .apply-content').animate({
					...
					//alert($('#apply-container form').serialize());
						$.ajax({
							url: 'pages/applyonline.php',
							data: $('#apply-container form').serialize() + '&action=forgotpass',
							type: 'post',
							cache: false,
							dataType: 'html',
							complete: function (abc) {
							var	res = abc.responseText;
								$('#apply-container .apply-loading').fadeOut(200, function () {
									...
									{
										$('#apply-container .apply-title').html("Forgot Password");
										showForgotDiv();
										//$('#apply-container .apply-loading').hide();
										//$('#apply-container .apply-message').html("Invalid Email / Password.").fadeIn(200);	
										$('#apply-container .apply-message').show();
										$('#apply-container .apply-message').html($('<div class="apply-error">').append("Sorry email does't exist.")).fadeIn(200);
										//$('#apply-container .apply-message1').fadeOut(500);
										//document.getElementById("s1").style.display = 'block';
										//$('#apply-container .apply-message').html("Email (user name) already taken.").fadeIn(200);	
									}
	...
	validate: function () {
		...
		else if(getDivId() == '3')
		{
			if (!$('#apply-container #apply-contactdesignation').val()) {
				...
				apply.message += 'Company/organisation name is required.';
			}
...
*/			
	...
	validatePassword: function () {
		apply.message = '';
		var email = $('#apply-container #apply-username-password').val();
		if (!email) {
			apply.message += 'Email is required. ';
		}
		else if (!apply.validateEmail(email)) {
		...
		else if (!$('#apply-container #apply-password-password').val()) {
			apply.message += 'Password is required. ';
		}
		else if ($('#apply-container #apply-password-password').val().length < 6) {
			apply.message += 'Password must be atleast 6 characters long. ';
		}
		else if (!$('#apply-container #apply-new-password').val()) {
			apply.message += 'New Password is required. ';
		}
		else if ($('#apply-container #apply-new-password').val().length < 6) {
			apply.message += 'New Password must be atleast 6 characters long. ';
		}
		else if (!$('#apply-container #apply-conf-new-password').val()) {
			apply.message += 'Confirm New Password is required. ';
		}
		else if ($('#apply-container #apply-conf-new-password').val().length < 6) {
			apply.message += 'Confirm New Password must be atleast 6 characters long. ';
		}
		else if ($('#apply-container #apply-conf-new-password').val() != $('#apply-container #apply-new-password').val()) {
			apply.message += 'New Password and Confirm New Password must match';
		}
	...
	validateForgotPass: function () {
		apply.message = '';
		var email = $('#apply-container #apply-forgotusername').val();
		if (!email) {
			apply.message += 'Email is required. ';
		}
		else if (!apply.validateEmail(email)) {
...
//		if(getDivId() == 8){
		...
		if(document.getElementById('spnProgrammeName') != null && typeof programmeName != 'undefined')
		{
			//document.getElementById('spnProgrammeName').innerHTML = '<span style="font-size:12px; font-weight:bolder; color:#000000;">Programme:</span> ' + programmeName;			
		}
...
*/		
		if (apply.validate()) 
		{
				...
				if(divid == 2)
				{
					var innerName = "Personal Data";	
				}
				if(divid == 3)
				{
					var innerName = "Contact Data";	
				}
				if(divid == 4)
				{
					var innerName = "Organisational Data";	
				}
				if(divid == 5)
				{
					var innerName = "Professional Data";	
				}
				if(divid == 6)
				{
					var innerName = "Sponsorship and Invoicing";	
				}
				if(divid == 7)
				{
					var innerName = "Information Source";	
				}
				
				$('#p' + divid).html('<a href="javascript:toggleFunc('+(divid - 1)+')" class="sharpLink">'+innerName+'</a>');
				...
				//document.getElementById("i"+i).src = 'images/applyonline/'+i+'_trans.gif';
							...
							//		if(getDivId() != 1)
						{
							...
							$.ajax({
										url: 'pages/applyonline.php',
										data: $('#apply-container form').serialize() + '&action=create&divnum='+getDivId()+"&pname=" + programmeName,
										type: 'post',
										cache: false,
										dataType: 'html',
										complete: function (xhr) {
														...
														{			
															//$('#apply-container form').fadeIn(200);	
																...
																if(i == divcurr)
																{
																	...
																	if(i<7)
																	{
//																		document.getElementById('spnProgrammeName'+i).innerHTML = '<span style="font-size:12px; font-weight:bolder; color:#000000;"></span> ' + programmeName;
																	}
																}
																else
																{
														...
														for(var j = 7; j>divcurr; j--)	
														{
																//$('#apply-container form').fadeIn(200);		
																document.getElementById("s"+j).style.display = 'none';
																if(j<8){
																	document.getElementById("p"+j).className = '';
																	document.getElementById("i"+j).src = 'images/applyonline/'+j+'_trans.gif';
																}
													...
													else if(divid = 7)
														{
															...
															{
																if(j == 7)
																	$('#p' + j).html("Information Source");
																
																if(j == 6)
																	$('#p' + j).html("Sponsorship and Invoicing");
																	
																if(j == 5)
																	$('#p' + j).html("Professional Data");
																
																if(j == 4)
																	$('#p' + j).html("Organisational Data");
																	
																if(j == 3)
																	$('#p' + j).html("Contact Data");	
																	
																if(j == 2)
																	$('#p' + j).html("Personal Data");	
...
//							document.getElementById('spnProgrammeName'+i).innerHTML = '<span style="font-size:12px; font-weight:bolder; color:#000000;"></span> ' + programmeName;
	...
	function setProgName(pname)
	{
		getProgrammeName();
//		document.getElementById('spnProgrammeName2').innerHTML = '<span style="font-size:12px; font-weight:bolder; color:#000000;"></span> ' + programmeName;
	}
