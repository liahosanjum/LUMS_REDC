...
require_once($GENERAL['ADMIN_DIR_LIB'].'/alumnimanagement.lib.php');
require_once($GENERAL['BASE_DIR_ROOT'].'/fckeditor/fckeditor.php');
// create alumni Management object
$controller =& new alumniManagement;
...
switch($_action) {
    ...
    case 'listprog':
        // adding an  entry
		$controller->pageview="list";		
	    $controller->displayForm($controller->getUserProgrammes($_REQUEST['aid']));
	    break;
    ...
    case 'submit':
		...
		else
		{
			...
			if($controller->isValidForm($_POST,$_FILES,$_mode)) 
			 {
				if($controller->updateEntry($_POST,$_GET['id'],$_FILES))
				{
					$controller->pageview="display";
					$controller->displayGird($controller->getEntries($_start,$_POST));
				}
