...
require_once($GENERAL['ADMIN_DIR_LIB'].'/ofpbrouchermanagement.lib.php');
// create Gallery Management object
$controller =& new ofpbrouchermanagement;
...
switch($_action) {
	...
	case 'edit':
    	...
    	$controller->displayForm($controller->editEntry($_GET['ofpbid']));
	    break;
		
	case 'del':
        // deleting an entry
		$controller->pageview="del";
		$controller->deleteEntry($_GET['ofpbid']);
    ...
    case 'submit':
		...
		else
		{
			...
			if($controller->isValidForm($_POST,$_FILES,$_mode)) 
			 {
				if($controller->updateEntry($_POST,$_GET['ofpbid'],$_FILES))
				{
					$controller->pageview="display";
					$controller->displayGird($controller->getEntries($_start,$_POST));
				}
