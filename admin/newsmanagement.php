...
switch($_action) {
	...
	case 'edit':
    	...
    	$controller->displayForm($controller->editEntry($_GET['nid']));
	    break;
		
	case 'del':
        // deleting an entry
		$controller->pageview="del";
		$controller->deleteEntry($_GET['nid']);
    ...
    case 'submit':
		...
		else
		{
			...
			if($controller->isValidForm($_POST,$_FILES,$_mode)) 
			 {
				if($controller->updateEntry($_POST,$_GET['nid'],$_FILES))
				{
					$controller->pageview="display";
					$controller->displayGird($controller->getEntries($_start,$_POST));
				}
