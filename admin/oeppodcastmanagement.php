...
require_once($GENERAL['ADMIN_DIR_LIB'].'/oeppodcastmanagement.lib.php');
require_once($GENERAL['BASE_DIR_ROOT'].'/fckeditor/fckeditor.php');
// create alumni Management object
$controller =& new oeppodcastManagement;
...
switch($_action) {
	...
	case 'edit':
    	...
    	$controller->displayForm($controller->editEntry($_GET['podcastid']));
	    break;
		case 'upload':
        // upload images
	...
	case 'del':
		...
		$controller->deleteEntry($_GET['podcastid']);
    ...
    case 'submit':
		...
		else
		{
			...
			if($controller->isValidForm($_POST,$_FILES,$_mode)) 
			 {
				if($controller->updateEntry($_POST,$_GET['podcastid'],$_FILES))
				{
					$controller->pageview="display";
