...
class brouchermanagement extends db{
	...
	var $emailname = "";	
	var $tablename="redc_enrollments_broucher_requests";
	var $sortcolumn="ebrid";
	...
	*/
	function editEntry($ebrid=0)
	{
		//$_query = "select c.*,u.first_name,u.last_name,u.user_type,u.email_address from ".$this->tablename." c, ".$this->table_user." u where c.user_id = u.user_id and c.id=".$id."";
		   $_query ="select * from ".$this->tablename." where ebrid = $ebrid";
    ...
    function getEntries($_start=0,$formvars) {
		...
		$paging = new Paginate();
		$paging->num= $this->numrows("select ebrid from ".$this->tablename);
		$paging->start=$_start;
...
//		$paging->limit = PAGESIZE;
	...
	function getClosedEntries($action,$_start=0,$formvars) {
		...
		{
    		$this->error="No existing brochure request found";
		}
        return $data;   
    }
	
	function deleteEntry($ebrid=0)
	{
				 
		$_query="delete from ".$this->tablename." where ebrid=$ebrid";
    ...
    function displayForm($formvars = array()) {
        ...
        $this->tpl->display('viewbroucherrequest.tpl');
    }
    /**
