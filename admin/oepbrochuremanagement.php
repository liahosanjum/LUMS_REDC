...
require_once($GENERAL['ADMIN_DIR_LIB'].'/oepbrochuremanagement.lib.php');
// create Gallery Management object
$controller =& new oepbrouchermanagement;
...
switch($_action) {
	...
	case 'edit':
    	...
    	$controller->displayForm($controller->editEntry($_GET['oepbid']));
	    break;
		
	case 'del':
        // deleting an entry
		$controller->pageview="del";
		$controller->deleteEntry($_GET['oepbid']);
    ...
    case 'submit':
		...
		else
		{
			...
			if($controller->isValidForm($_POST,$_FILES,$_mode)) 
			 {
				if($controller->updateEntry($_POST,$_GET['oepbid'],$_FILES))
				{
					$controller->pageview="display";
					$controller->displayGird($controller->getEntries($_start,$_POST));
				}
