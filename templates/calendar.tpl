...
<title>{*$pagedata.explorertitle*}Programme Calendar</title>
<meta name="keywords" content="{$pagedata.keywords}" />
...
<!--				</div>-->
    ...
    <div class="center_pane_full">
      ...
      <div class="main_heading_cms" style="background:url({$GENERAL.FRONT_UPLOAD_URL}/homeSectionPictures/{$simage.0.sec_image}) no-repeat;"><h1>{*$pagedata.pagetitle*}Programme Calendar</h1></div>
        	<!--<div class="main_heading" style="background:url(images/banr.gif) no-repeat;"><h1>{$pagedata.pagename}</h1></div>-->
            <div class="programme_details" style="display:{if $pagedata.details eq ""}none{/if}" >
			{$pagedata.details}			
			</div>
			...
			{/if}
                ...
                <li class="{$class} detail ">
				 ...
				 <li class="{$class} detail ">
                    ...
                    {if $entry.status ne 'tba'}
					<a class="list1" href="{$GENERAL.BASE_URL_ROOT}/apply.php?pid={php} echo encrypt($this->_tpl_vars['entry']['oepid']);{/php}#apply">Apply</a>{else}<span style="font-weight:bold">Apply</span>
                    {/if}
                 </li>
				{/foreach}
        ...
        <div style="width:100%; clear:left; display:{if $pagenum < 10}none;{/if}">{$paging}</div>
    </div>
